...
library(dplyr)
library(purrr)
#The script is written by George Buzzell 
...
proc_fileName <- "2021-11-05_flanker_behavior_summary.csv"
...
summaryDat <- setNames(data.frame(matrix(ncol = 10, nrow = 0)), c("id", "con_Acc_mean", "incon_Acc_mean", "conCorr_RT_mean", "inconCorr_RT_mean", "conCorr_logRT_mean", "inconCorr_logRT_mean", "flankEff_Acc", "flankEff_RT", "flankEff_logRT"))
...
for(i in 1:length(sub_folders)) {
  ...
  if (!identical(flanker_fileName, character(0))) {
    ...
    id <- testDatTrim$id[1]
    summaryDat[nrow(summaryDat) + 1,] <-c(id, con_Acc_mean, incon_Acc_mean, conCorr_RT_mean, inconCorr_RT_mean, conCorr_logRT_mean, inconCorr_logRT_mean, flankEff_Acc, flankEff_RT, flankEff_logRT)
