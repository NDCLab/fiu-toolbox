...
summaryDat <- setNames(data.frame(matrix(ncol = 10, nrow = 0)), c("id",
                                                                  "rep_Acc_mean", 
                                                                  "no_rep_Acc_mean", 
                                                                  "repCorr_RT_mean", 
                                                                  "no_repCorr_RT_mean", 
                                                                  "repCorr_logRT_mean", 
                                                                  "no_repCorr_logRT_mean", 
                                                                  "nbackEff_Acc", 
                                                                  "nbackEff_RT", 
                                                                  "nbackEff_logRT"))
...
for(i in 1:length(sub_folders)) {
  ...
  #for this participant, find the n-back file name
  ...
  #logical to make sure there is a n-back file for this participant before loading, else, skip to next participant
  if (!identical(nback_fileName, character(0))) {
    ...
    #subset the data for correct trials only, separately for repeat and no repeat trials, creating new data frames for each
    ...
    repCorr_RT_mean <- mean(testDatTrim_repCorr$key_resp.rt)
    no_repCorr_RT_mean <- mean(testDatTrim_no_repCorr$key_resp.rt)
...
library(lsr)
t.test(summaryDat$no_rep_Acc_mean, summaryDat$rep_Acc_mean, alternative = "two.sided", paired = TRUE)
t.test(summaryDat$no_repCorr_logRT_mean, summaryDat$repCorr_logRT_mean, alternative = "two.sided", paired = TRUE)