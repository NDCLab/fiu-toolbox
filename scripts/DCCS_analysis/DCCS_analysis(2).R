...
chosen_cols <- data.frame(dccs[, 60])
...
names(chosen_cols) <- c('keys', 'correct_resp', 'rt', 'cue')
...
results = rbind(results, data.frame(switch_cost_RT, switch_cost_RT_log, switch_cost_corr))
}
